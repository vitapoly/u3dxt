...
namespace U3DXT.iOS.GameKit {
	...
	public class TurnBasedMatchesController {
		...
		private static EventHandler<ExchangeEventArgs> _exchangeRequestReceivedHandlers;
		/// <summary>
		/// Occurs when the user receives an exchange request.
		/// </summary>
		public static event EventHandler<ExchangeEventArgs> ExchangeRequestReceived {
			add { _exchangeRequestReceivedHandlers += value; }
			remove { _exchangeRequestReceivedHandlers -= value; }
		}

		private static EventHandler<ExchangeEventArgs> _exchangeCancelledHandlers;
		/// <summary>
		/// Occurs when the user receives an exchange cancellation.
		/// </summary>
		public static event EventHandler<ExchangeEventArgs> ExchangeCancelled {
			add { _exchangeCancelledHandlers += value; }
			remove { _exchangeCancelledHandlers -= value; }
		}

		private static EventHandler<ExchangeEventArgs> _exchangeCompletedHandlers;
		/// <summary>
		/// Occurs when the user receives all replies to the exchange request.
		/// </summary>
		public static event EventHandler<ExchangeEventArgs> ExchangeCompleted {
			add { _exchangeCompletedHandlers += value; }
			remove { _exchangeCompletedHandlers -= value; }
		}
