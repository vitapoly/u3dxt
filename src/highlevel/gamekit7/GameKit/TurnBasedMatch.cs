...
namespace U3DXT.iOS.GameKit {
	...
	public class TurnBasedMatch : GameCenterMatch {
		...
		private EventHandler<ExchangeSentEventArgs> _exchangeSentHandlers;
		/// <summary>
		/// Occurs when sending an exchange is completed.
		/// </summary>
		public event EventHandler<ExchangeSentEventArgs> ExchangeSent {
			add { _exchangeSentHandlers += value; }
			remove { _exchangeSentHandlers -= value; }
		}

		private EventHandler<EventArgs> _sendReminderCompletedHandlers;
		/// <summary>
		/// Occurs when the send reminder operation completed successfully.
		/// </summary>
		public event EventHandler<EventArgs> SendReminderCompleted {
			add { _sendReminderCompletedHandlers += value; }
			remove { _sendReminderCompletedHandlers -= value; }
		}

		private EventHandler<U3DXTErrorEventArgs> _sendReminderFailedHandlers;
		/// <summary>
		/// Occurs when the send reminder operation failed.
		/// </summary>
		public event EventHandler<U3DXTErrorEventArgs> SendReminderFailed {
			add { _sendReminderFailedHandlers += value; }
			remove { _sendReminderFailedHandlers -= value; }
		}

		private EventHandler<EventArgs> _saveMergedMatchDataCompletedHandlers;
		/// <summary>
		/// Occurs when the save merged data operation completed successfully.
		/// </summary>
		public event EventHandler<EventArgs> SaveMergedMatchDataCompleted {
			add { _saveMergedMatchDataCompletedHandlers += value; }
			remove { _saveMergedMatchDataCompletedHandlers -= value; }
		}

		private EventHandler<U3DXTErrorEventArgs> _saveMergedMatchDataFailedHandlers;
		/// <summary>
		/// Occurs when the save merged data operation failed.
		/// </summary>
		public event EventHandler<U3DXTErrorEventArgs> SaveMergedMatchDataFailed {
			add { _saveMergedMatchDataFailedHandlers += value; }
			remove { _saveMergedMatchDataFailedHandlers -= value; }
		}
	...
	//			Debug.Log("ReloadPlayers IDs: " + Json.Serialize(playerIDs));
		...
		public GKTurnBasedExchange[] activeExchanges {
			get {
				var ex = gkTurnBasedMatch.activeExchanges;
				return (ex != null) ? ex.Cast<GKTurnBasedExchange>().ToArray() : null;
			}
		}
		...
		public GKTurnBasedExchange[] completedExchanges {
			get {
				var ex = gkTurnBasedMatch.completedExchanges;
				return (ex != null) ? ex.Cast<GKTurnBasedExchange>().ToArray() : null;
			}
		}
		...
		public GKTurnBasedExchange[] exchanges {
			get {
				var ex = gkTurnBasedMatch.exchanges;
				return (ex != null) ? ex.Cast<GKTurnBasedExchange>().ToArray() : null;
			}
		}
